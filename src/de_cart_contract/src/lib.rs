...
use ic_stable_structures::{
    ...
    DefaultMemoryImpl, StableLog, StableVec,
};

use ic_cdk::{
    api, api::stable::WASM_PAGE_SIZE_IN_BYTES, export_candid, post_upgrade, pre_upgrade, storage,
    update,
};
...
fn pre_upgrade() {
    ...
    storage::stable_save((marketplace,)).unwrap();
}

#[post_upgrade]
fn post_upgrade() {
    let (marketplace,): (Marketplace,) = storage::stable_restore().unwrap();
...
#[ic_cdk::query]
fn hello() -> String {
    "Hello from another upgrade I made".to_string()
}
